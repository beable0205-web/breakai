...
export default function ScoreGauge({ scoreObj, scoreColor }: { scoreObj: ScoreObj, scoreColor: string }) {
    ...
    let label = "NEUTRAL";
    if (score >= 80) { strokeColor = "#00FF41"; label = "EXTREME GREED (BUY)"; }
    else if (score >= 60) { strokeColor = "#4ade80"; label = "GREED (BUY)"; }
    else if (score <= 20) { strokeColor = "#ef4444"; label = "EXTREME FEAR (SELL)"; }
    else if (score <= 40) { strokeColor = "#f87171"; label = "FEAR (SELL)"; }
            ...
            {isOpen && mounted && createPortal(
                <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-8 md:p-12 w-full max-w-3xl shadow-2xl relative">
                        ...
                        <h2 className="text-2xl font-bold text-white mb-6">Investment Score Breakdown</h2>
                        ...
                        <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
                            ...
                            {(!scoreObj.breakdown || scoreObj.breakdown.length === 0) && (
                                <p className="text-gray-400 text-center py-4">No breakdown data available.</p>
                            )}
                        </div>
                    </div>
                </div>,
