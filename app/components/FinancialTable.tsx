            ...
            <table className="w-full text-sm text-left font-mono">
                ...
                <tbody className="divide-y divide-[#27272a] bg-[#18181b]">
                    {Object.entries(data).map(([key, value]) => {
                        ...
                        const isPositive = value.includes('+') || value.toLowerCase().includes('up') || value.toLowerCase().includes('expand');
                        const isNegative = value.includes('-') || value.toLowerCase().includes('down') || value.toLowerCase().includes('contract') || value.toLowerCase().includes('high'); // High debt/risk
